# It has static selected options of garment with separate blocks
# app.py
import numpy as np
...
class LeffaPredictor(object):
    ...
    def leffa_predict_vt(
        ...
        """
        Virtual try-on inference wrapper.
        """
        # Validate inputs
        if not src_image_path:
            raise gr.Error("Please upload/capture a Person Image.")
        if not ref_image_path:
            raise gr.Error("Please upload/capture a garment image (Upper/Lower/Dress).")
...
if __name__ == "__main__":
    ...
    with gr.Blocks(css=custom_css, theme=gr.themes.Soft(), title="Vimal AI Outfit App") as demo:
        ...
        <section class="hero">
          ...
          <p>Upload or capture your photo and garment to see AI-powered try-on results.<br>
          Professional-grade technology for realistic clothing visualization.</p>
        </section>
        """)
        ...
        with gr.Row(elem_classes="grid"):
            ...
            with gr.Column(elem_classes="card"):
                ...
                vt_garment_type = gr.Radio(
                    label="Garment Type (select which block to use)",
        ...
        # Internal helper: pick correct garment path by selected type
        def _pick_garment_path(gtype, up, low, drs):
            if gtype == "upper_body":
                return up or ""
            if gtype == "lower_body":
                return low or ""
        ...
        def run_vton(
            src_path, gtype, up_path, low_path, drs_path,
            vt_ref_acceleration, vt_step, vt_scale, vt_seed, vt_model_type, vt_garment_type, vt_repaint
        ):
            garment_path = _pick_garment_path(gtype, up_path, low_path, drs_path)
            return leffa_predictor.leffa_predict_vt(
                src_path, garment_path,
                vt_ref_acceleration, int(vt_step), float(vt_scale), int(vt_seed),
        ...
        gr.Markdown(
            ...
            "Note: Models are trained on academic datasets (VITON-HD / DressCode) for research/demo purposes."
            "</div>"
        )

        demo.launch(share=True, pwa=True, server_port=7860, allowed_paths=["./ckpts/examples"])
